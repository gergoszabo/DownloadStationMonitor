...
if (isset($_SESSION['doConfig'])) {
    header('Location: ' . getBaseUrl() . '?config');
    exit();
}
...
foreach ($tasks['data']['tasks'] as $task) {
    if ($_SESSION['config']['mod_simple'] && getTrackerStatusPriority(getTrackerStatus($task)) == TRACKER_STATUS_OK) {
        continue;
    }
...
$page = str_replace('##REFRESH##', $hasIntermediateStatus ? 2 : $_SESSION['config']['ujratoltes'], $page);
$page = str_replace('##BODY_THEME##', ($_SESSION['config']['dark'] ? 'bg-dark text-light' : 'bg-light text-dark'), $page);
$page = str_replace('##TABLE_THEME##', ($_SESSION['config']['dark'] ? 'table-dark' : 'table-light'), $page);
...
$page = str_replace("##SIMPLE##", $_SESSION['config']['mod_simple'] ? 'Simple ' : '', $page);

$page = str_replace('##RSS##', $_SESSION['config']['rss'] ? ' <a class="btn btn-outline-info" href="?rss">RSS</a>' : '', $page);
